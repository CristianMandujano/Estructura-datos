...
class Graph:
    ...
    def contains_vertex(self, vertex):
        return self._normalize_vertex(vertex) in self._adjacency_list
    ...
    def get_adjacency_list(self):
        return {v: self.get_adjacent_vertices(v) for v in self.get_vertices()}

    def get_adjacency_matrix(self):
        vertices = self.get_vertices()
        matrix = []
        for row_v in vertices:
            row = [1 if self.contains_edge(row_v, col_v) else 0 for col_v in vertices]
            matrix.append(row)
    ...
    def get_edges(self):
        ...
        for v1 in self.get_vertices():
            for v2 in self.get_adjacent_vertices(v1):
                edge = tuple(sorted((v1, v2)))
                if edge not in registered:
                    registered.add(edge)
                    edges.append(edge)
        return sorted(edges)
